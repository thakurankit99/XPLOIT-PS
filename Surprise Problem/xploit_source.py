...
def _xb(d, k = (_XK,)):
    return None((lambda .0 = None: for b in .0:
b ^ k)(d))
...
def _gf(n):
    if n not in _fc:
        _fc[n] = pygame.font.SysFont('monospace', n, True, **('bold',))
    return _fc[n]
...
def _ht(s, t, x, y, c, ctr, sz = (False, 20)):
    f = _gf(sz)
...
class _Eq:
    ...
    def _fs(self, k):
        ...
        if k[pygame.K_SPACE] or self.sc == 0:
...
class _Ox:
    ...
    def _v(self, s):
        ...
        lbl = _sf().render(''.join((lambda .0: for o in .0:
chr(o))([
            79,
            80,
            69,
...
class _Tx:
    ...
    def _v(self, s):
        ...
        pygame.draw.rect(s, c1, self.r, 2)
        s.blit(_sf().render(''.join((lambda .0: for o in .0:
chr(o))((69, 88, 73, 84))), True, c0), (self.r.x + 4, self.r.y + 12))
...
class _Ux:
    ...
    def _u(self, p):
        if math.hypot(p.r.centerx - self.r.centerx, p.r.centery - self.r.centery) < 50:
...
class _Gx:
    ...
    def _v(self, s):
        ...
        s.blit(_sf().render(''.join((lambda .0: for o in .0:
chr(o))((66, 79, 83, 83))), True, c1), (self.r.x + 6, self.r.y + 20))
...
class _S1(_Mx):
    _NM = ''.join((lambda .0: for c in .0:
chr(c))((83, 116, 97, 103, 101, 32, 49)))
    
    def __init__(self = None, p = None):
        super().__init__()
    ...
    def _u(self, p, ev):
        ...
        p._ck()
        for c in self.cv:
            if c.cd and p.r.colliderect(c.r):
                c.cd = True
                p.cn += 1
        self.dv._c(p)
    ...
    def _n(self, s, p):
        _ht(s, f'''{p.cn}''', 10, 10, c7, 20, **('sz',))
        _ht(s, self._NM, _W // 2, 10, c1, True, **('ctr',))
...
chr(c))((83, 116, 97, 103, 101, 32, 50)))
    ...
    def _v(self, s):
        ...
        if self.go:
            ...
            ov.fill((180, 0, 0, 160))
            s.blit(ov, (0, 0))
            _ht(s, ''.join((lambda .0: for o in .0:
chr(o))((84, 69, 82, 77, 73, 78, 65, 84, 69, 68))), _W // 2, _H // 2, c1, True, 42, **('ctr', 'sz'))
            _ht(s, ''.join((lambda .0: for o in .0:
chr(o))((80, 65, 84, 67, 72, 32, 65, 78, 68, 32, 82, 69, 84, 82, 89))), _W // 2, _H // 2 + 50, c7, True, 22, **('ctr', 'sz'))
            return None
    ...
    def _n(self, s, p):
        ...
        cl = c5
        _ht(s, f'''{t:.2f}''', _W // 2, 10, cl, True, 32, **('ctr', 'sz'))
        ...
        _ht(s, self._NM, _W // 2, _H - 28, c1, True, **('ctr',))
...
chr(c))((83, 116, 97, 103, 101, 32, 51)))
    ...
    def _v(self, s):
        ...
        vl = _sf().render(''.join((lambda .0: for o in .0:
chr(o))((86, 79, 73, 68))), True, (50, 50, 50))
...
chr(c))((83, 116, 97, 103, 101, 32, 52)))
    ...
    def _u(self, p, ev):
        ...
        for b in p.bl:
            b._u()
        p.bl = (lambda .0: [ b for b in .0 if b.ac ])(p.bl)
        if not self.bx.dd:
            self.bx._u(self.pf)
            for b in p.bl:
        ...
        if p.r.top > _H + 100:
            p.r.topleft = (60, _H - _T - p._SH)
            p.vy = 0
        if self.bx.dd or p.r.colliderect(self.xb.r):
            self.cp = True
            return None
        return None
    ...
    def _n(self, s, p):
        _ht(s, '[SPC]', 10, 10, c9, 16, **('sz',))
        _ht(s, self._NM, _W // 2, 10, c1, True, **('ctr',))
        if self.bx.dd:
            _ht(s, ''.join((lambda .0: for o in .0:
chr(o))((80, 82, 79, 67, 69, 69, 68))), _W // 2, _H // 2 - 40, c5, True, 26, **('ctr', 'sz'))
            return None
...
chr(c))((83, 116, 97, 103, 101, 32, 54)))
    ...
    def __init__(self = None, p = None):
        ...
        self._bw = []
        for rx, ry, rw, rh in ((self._BX, self._BY, self._BW, self._WT), (self._BX, self._BY + self._BH - self._WT, self._BW, self._WT), (self._BX, self._BY, self._WT, self._BH), (self._BX + self._BW - self._WT, self._BY, self._WT, self._BH)):
    ...
    def _n(self, s, p):
        _ht(s, self._NM, _W // 2, _H - 28, c1, True, **('ctr',))
        _ht(s, ''.join((lambda .0: for o in .0:
chr(o))((63, 63, 63))), _W // 2, _H // 2 - 20, c2, True, 48, **('ctr', 'sz'))
...
def _encode_flag(n):
    return bytes((lambda .0: for i, b in .0:
b ^ (_XK + i) % 256)(enumerate(n.encode())))
...
def _scr_t(s, cl):
    s.fill((10, 10, 20))
    for t, sz, c, y in ((_LBL_G, 42, c9, _H // 2 - 120), (_decode_str([
        82,
        69,
        86,
        69,
        ...
        65,
        67,
        69]), 18, c2, _H // 2 + 70), (_LBL_E, 24, c5, _H // 2 + 120)):
        _ht(s, t, _W // 2, y, c, True, sz, **('ctr', 'sz'))
...
def _scr_w(s, cl):
    s.fill((5, 20, 5))
    for t, sz, c, y in ((_LBL_W, 36, c5, _H // 2 - 80), (_decode_str([
        65,
        76,
        76,
        32,
        ...
        69,
        68,
        46]), 22, c1, _H // 2 - 20), (_LBL_S, 20, c9, _H // 2 + 20), (_LBL_E, 20, c2, _H // 2 + 80)):
        _ht(s, t, _W // 2, y, c, True, sz, **('ctr', 'sz'))
    pygame.display.flip()
    cl.tick(_F)
    for e in pygame.event.get():
...
def _scr_r(s, cl, nm):
    s.fill((5, 5, 15))
    _ht(s, _LBL_C, _W // 2, _H // 2 - 50, c5, True, 40, **('ctr', 'sz'))
    _ht(s, nm, _W // 2, _H // 2 + 10, c1, True, 22, **('ctr', 'sz'))
    _ht(s, _LBL_E, _W // 2, _H // 2 + 60, c2, True, 18, **('ctr', 'sz'))
    pygame.display.flip()
    cl.tick(_F)
    for e in pygame.event.get():
        if e.type == pygame.QUIT:
